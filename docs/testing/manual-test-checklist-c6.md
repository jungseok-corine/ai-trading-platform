...
## L. C-6.21 KIS 실시간 웹소켓 (장중 라이브 검증 필요)

- [ ] `.env`에 `KIS_WS_ENABLED=true` (+ 필요 시 `KIS_WS_SYMBOLS=005930,000660`) + 재시작.
- [ ] **장중** 서버 로그에 "KIS WS 연결 — N종목 구독" 확인.
- [ ] 1~3분 후 market_data 1m 신규 행이 REST 폴링 주기와 무관하게 매분 들어오는지:
  `SELECT ts FROM market_data WHERE timeframe='1m' AND symbol_code='005930' ORDER BY ts DESC LIMIT 5;`
- [ ] 필드 인덱스 검증: WS로 들어온 1분봉 OHLCV가 REST 1분봉과 대략 일치하는지 (가격 자릿수/거래량).
