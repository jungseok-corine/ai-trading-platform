...
다음 후보(데이터가 쌓이면):
0. **C-4 전략 엔진 확장** (설계: `docs/design/C-4-strategy-engine.md`) — 엔진이 이동평균
   교차밖에 못 도는 현 상태를 깨고 실전 전략 타입 4종(RSI 평균회귀·MACD 추세·돌파·눌림목) 추가.
   서로 다른 매매 원리로 로그를 차별화 → AI 제안·회고 루프의 입력을 풍부하게. 기존 토이 전략은
   아카이브 + 로그 리셋으로 깨끗한 측정 기준선 시작.
