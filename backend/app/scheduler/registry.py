...
from app.scheduler.jobs import (
    ...
    STRATEGY_RESEARCHER_JOB_ID,
    run_strategy_researcher_job,
...
CONTROLLABLE_JOBS: list[ControllableJob] = [
    ...
    ControllableJob(
        job_id=STRATEGY_RESEARCHER_JOB_ID,
        label_ko="LLM 전략 리서처 (C-7.6)",
        schedule_desc="매주 월 08:20",
        func=run_strategy_researcher_job,
        build_trigger=lambda s: CronTrigger(day_of_week="mon", hour=8, minute=20),
        env_enabled=lambda s: s.strategy_researcher_scheduler_enabled,
        description="LLM이 유명 매매 전략을 DSL 스펙으로 발굴 → 검증·입학시험 → 통과분만 pending 제안.",
        risk_level="KEEP_OFF",
        recommended_state="KEEP_OFF",
        writes_db=True,
        uses_llm=True,
        ...
        creates_proposals=True,
        cost_risk=True,
        safety_notes=("LLM 유료 호출", "코드 실행 없음(DSL만)", "pending 제안만 — 승인은 사람", _NO_LIVE),
    ),
    ControllableJob(
